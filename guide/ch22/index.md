...
<span class="tool">humsed</span> command and learn how it can be used
...
can use the <span class="tool">xdelta</span> command to calculate the
...
136. The <span class="tool">recode</span> command allows us to
...
exceed some threshold. The <span class="tool">recode</span> command requires that the user
...
*Xheart
  ...
  \*-
  -----------
Notice that we have used <span class="tool">recode</span> to drastically reduce the volume of
...
<span class="tool">recode</span> include the following:
...
<span class="tool">semits</span>, translate to semitone-differences
...
notes played in the throat register. We can use <span class="tool">recode</span> to classify
...
more succinct <span class="tool">humsed</span> script would deal with fingering transitions
...
aspects of the music. For example, we might use <span class="tool">yank</span> to extract
...
<span class="tool">recode</span> command and non-parametric
classification can be achieved using the *substitution* operation
provided by the <span class="tool">humsed</span> command.
