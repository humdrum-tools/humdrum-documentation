...
filenames produced by the <span class="unix">grep</span> command are assigned to a shell
...
executed *before* the <span class="unix">sort</span> command. The **grep -l** command will
generate as output a string of filenames. This output will replace the
material delineated by greves. Finally, the <span class="unix">sort</span> command will be
executed &mdash; using the filenames generated by the <span class="unix">grep</span> command.
...
Here we have imbedded one <span class="unix">grep</span> "inside" another. Remember that the
...
<span class="unix">grep</span> is executed. It looks for files that contain a `COM` reference
record containing the keyword "`Josquin`." But this second <span class="unix">grep</span>
only searches those filenames passed to it by the first <span class="unix">grep</span>. In
...
The UNIX <span class="unix">echo</span> command causes information to be printed or displayed.
...
<span class="unix">grep</span> is used to assign these line numbers to the shell variables
`$A` and `$B`.
...
<span class="unix">alias</span> command. In this case, we've defined a new command called
`inventory`:
...
We have also learned that the shell <span class="unix">alias</span> command can be used to
